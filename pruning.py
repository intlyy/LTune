...
knobs_20 = """
    here are the 20 most important knobs to consider for tuning:\n
    1.**innodb_buffer_pool_size**: This is crucial for caching table and index data, which can significantly impact read and write performance.\n   
    n2. **innodb_log_file_size**: Affects the performance of write operations and recovery time.\n   
    3. **innodb_flush_log_at_trx_commit**: Controls the durability of transactions and can impact write throughput.\n  
    4. **innodb_thread_concurrency**: Limits the number of threads that can enter InnoDB, affecting concurrency.\n   
    5. **innodb_io_capacity**: Determines the I/O capacity for background operations, impacting write performance.\n 
    6. **innodb_read_io_threads**: Number of I/O threads for read operations, affecting read throughput.\n  
    7. **innodb_write_io_threads**: Number of I/O threads for write operations, affecting write throughput.\n 
    8. **innodb_max_dirty_pages_pct**: Controls the percentage of dirty pages in the buffer pool, impacting flushing behavior.\n  
    9. **innodb_adaptive_flushing**: Helps in maintaining a steady state of flushing, impacting write performance.\n   
    10. **innodb_flush_neighbors**: Affects the flushing of pages, which can impact I/O performance.\n    
    11. **innodb_adaptive_hash_index**: Can improve performance for certain workloads by speeding up index lookups.\n    
    12. **innodb_change_buffering**: Buffers changes to secondary indexes, impacting write performance.\n    
    13. **innodb_doublewrite**: Provides data integrity but can be disabled for performance gains at the risk of data loss.\n   
    14. **innodb_lru_scan_depth**: Influences the flushing algorithm, impacting buffer pool performance.\n    
    15. **innodb_purge_threads**: Number of threads for purge operations, affecting background processing.\n   
    16. **innodb_page_cleaners**: Number of page cleaner threads, impacting the flushing of dirty pages.\n   
    17. **innodb_spin_wait_delay**: Affects the performance of spin locks, impacting concurrency.\n   
    18. **innodb_flush_log_at_timeout**: Controls the frequency of log flushing, impacting durability and performance.\n   
    19. **innodb_adaptive_max_sleep_delay**: Allows InnoDB to adjust sleep delay based on workload, impacting performance.\n   
    20. **innodb_old_blocks_time**: Protects the buffer pool from being filled with transient data, impacting cache efficiency.\n    
"""
...
def call_open_source_llm_1(model,knob_list):
    ...
    for choice in completion.choices:
        ...
        #print(parsed_data)
        with open('./knob/opt_space.json',"a") as f:
